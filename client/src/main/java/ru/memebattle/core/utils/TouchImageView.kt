...
class TouchImageView(context: Context, attributeSet: AttributeSet) :
    ...
    init {
        ...
        maxScale = 5f
        superMinScale =
            SUPER_MIN_MULTIPLIER * minScale
        superMaxScale =
            SUPER_MAX_MULTIPLIER * maxScale
        imageMatrix = mMatrix
    ...
    fun setZoom(
        ...
        m!![Matrix.MTRANS_Y] =
            -(focusY * imageHeight - viewHeight * 0.5f)
        mMatrix!!.setValues(m)
        fixTrans()
        imageMatrix = mMatrix
    }
    ...
    private fun fitImageToView() {
        ...
        if (!isZoomed && !imageRenderedAtLeastOnce) { //
            ...
            //
            m!![Matrix.MSCALE_X] =
                matchViewWidth / drawableWidth * currentZoom
            m!![Matrix.MSCALE_Y] =
                matchViewHeight / drawableHeight * currentZoom
            //
    ...
    private fun translateMatrixAfterRotate(
        ...
        when {
            ...
            trans > 0 -> {
                ...
                //
                val percentage =
                    (abs(trans) + 0.5f * prevViewSize) / prevImageSize
            ...
            if (state == State.NONE || state == State.DRAG || state == State.FLING) {
                when (event.action) {
                    ...
                    MotionEvent.ACTION_MOVE -> if (state == State.DRAG) {
                        ...
                        val fixTransX =
                            getFixDragTrans(deltaX, viewWidth.toFloat(), imageWidth)
                        val fixTransY =
                            getFixDragTrans(deltaY, viewHeight.toFloat(), imageHeight)
                        mMatrix!!.postTranslate(fixTransX, fixTransY)
                        fixTrans()
                        last[curr.x] = curr.y
                    }
                    MotionEvent.ACTION_CANCEL -> {
                        run { Log.i("TAG", "ACTION_CANCEL") }
                        run { Log.i("TAG", "ACTION_UP") }
     ...
     * @author Ortiz
     */
    private inner class Fling internal constructor(velocityX: Int, velocityY: Int) :
        Runnable {
    ...
    companion object {
        ...
        //
        private const val SUPER_MIN_MULTIPLIER = .25f
        private const val SUPER_MAX_MULTIPLIER = 5f
    }
}